    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
            ...
            {continueVideo && (
              <Link href={`/student/watch/${continueVideo.id}`}>
                <Button size="lg" variant="secondary" className="mt-4">
                  ▶ Continue learning — {continueVideo.title}
                </Button>
              </Link>
            )}
          </div>
          <div className="glass rounded-3xl p-5 text-center text-forest-900">
            ...
            <p className="display text-4xl font-bold">⭐ {points}</p>
            <div className="mt-2 flex justify-center gap-1 text-2xl">
              {myBadges.slice(0, 4).map((b) => <span key={b!.id} title={b!.name}>{b!.emoji}</span>)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl bg-white p-6 shadow-soft ring-1 ring-black/5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <JourneyStat label="Reels watched" value={progress.length} emoji="🎬" />
          <JourneyStat label="Badges earned" value={myBadges.length} emoji="🏅" />
...
function JourneyStat({ label, value, emoji }: { label: string; value: number; emoji: string }) {
  return (
    <div className="rounded-2xl bg-cream/60 p-4">
