  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard label="Reels watched" value={a.videosWatched} icon={<span>🎬</span>} />
        <StatCard label="Total watch time" value={formatWatchTime(a.totalWatchTime)} icon={<span>⏱️</span>} tone="mist" />
        <StatCard label="Avg quiz score" value={`${a.avgQuiz}%`} icon={<span>✅</span>} tone="gold" />
        <StatCard label="Worksheets done" value={a.worksheetsCompleted} icon={<span>📝</span>} tone="clay" />
      </div>
          ...
          {a.rows.map((p) => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-soft ring-1 ring-black/5">
                <span className="text-2xl">{getVideo(p.videoId)?.thumbEmoji ?? "🎬"}</span>
